...
with st.sidebar:
    ...
    if not use_demo or not demo_mode_enabled or remaining_demos <= 0:
        ...
        if not demo_mode_enabled or remaining_demos <= 0:
            ...
            st.markdown("1. Visit [OpenRouter](https://openrouter.ai/)")
            st.markdown("2. Sign up and add credits")
            st.markdown("3. Generate an API key")
            st.markdown("**Cost**: ~$0.10-$0.30 per paper")
    else:
        user_api_key = ""
...
if st.button("🔍 Find Matching Journals", type="primary", disabled=not can_run):
    ...
    if using_demo and st.session_state.demo_uses >= MAX_DEMO_USES:
        st.info("🎉 You've used all your free trials! To continue using Journal Matcher, please add your own OpenRouter API key in the sidebar.")
...
st.markdown("---")
if demo_mode_enabled:
    st.markdown("*Demo mode enabled. Try it for free, then add your own API key for unlimited use.*")
else:
